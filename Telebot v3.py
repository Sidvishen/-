...
@dp.message_handler(text =['Привет', 'привет','хай','Хай'])
async def start(message):
    await message.answer('Нажмите на команду /communication, чтобы начать общение.')
@dp.message_handler(commands=['communication'])
async def start_message(message):
    await message.answer('Привет! Я бот помогающий твоему здоровью.'
                         'Вы можете нажать Calories для расчета коллорий.', reply_markup=kb)
@dp.message_handler( text = 'Информация')
async def inform(message):
    await message.answer('Привет! Я бот помогающий твоему здорью '
